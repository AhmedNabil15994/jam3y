...
<script>
  function tableGenerate(data='') {
    ...
    $('#dataTable').DataTable({
        ajax : {
            url   : "{{ url(route("course_groups.dataTable")) }}",
            type  : "GET",
            data  : {
                req : data,
            },
        ...
        columnDefs: [
    			...
    			},
          {
            ...
            width: '13%',
            title: '{{__('dashboard.course_groups.datatable.options')}}',
            className: 'dt-center',
            orderable: false,
            render: function(data, type, full, meta) {

              // Edit
    					var editUrl = '{{ route("course_groups.edit", ":id") }}';
    					editUrl = editUrl.replace(':id', data);

    					// Delete
    					var deleteUrl = '{{ route("course_groups.destroy", ":id") }}';
    					deleteUrl = deleteUrl.replace(':id', data);

    					return `
    					@permission('edit_course_groups')
