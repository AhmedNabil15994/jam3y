...
<script>
  function tableGenerate(data='') {
    ...
    $('#dataTable').DataTable({
        ajax : {
            url   : "{{ url(route('coupons.dataTable')) }}",
            type  : "GET",
            data  : {
                req : data,
            },
        ...
        columns: [
          ...
          {data: 'code' 			      , className: 'dt-center'},
    			{data: 'is_fixed_price'   , className: 'dt-center'},
          {data: 'price' 			      , className: 'dt-center'},
          {data: 'percent' 			    , className: 'dt-center'},
          {data: 'expire_date' 			, className: 'dt-center'},
        ...
        columnDefs: [
    			...
    			},
          {
            ...
            width: '13%',
            title: '{{__('dashboard.coupons.datatable.options')}}',
            className: 'dt-center',
            orderable: false,
            render: function(data, type, full, meta) {

              // Edit
    					var editUrl = '{{ route("coupons.edit", ":id") }}';
    					editUrl = editUrl.replace(':id', data);

    					// Delete
    					var deleteUrl = '{{ route("coupons.destroy", ":id") }}';
