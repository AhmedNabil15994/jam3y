...
<script>
  function tableGenerate(data='') {
    ...
    $('#dataTable').DataTable({
        ajax : {
            url   : "{{ url(route("attachments.dataTable")) }}",
            type  : "GET",
            data  : {
                req : data,
            },
        ...
        columnDefs: [
    			...
    			},
          {
            ...
            width: '13%',
            title: '{{__('dashboard.attachments.datatable.options')}}',
            className: 'dt-center',
            orderable: false,
            render: function(data, type, full, meta) {

              // Edit
    					var editUrl = '{{ route("attachments.edit", ":id") }}';
    					editUrl = editUrl.replace(':id', data);

    					// Delete
    					var deleteUrl = '{{ route("attachments.destroy", ":id") }}';
    					deleteUrl = deleteUrl.replace(':id', data);

    					return `
    					@permission('edit_attachments')
