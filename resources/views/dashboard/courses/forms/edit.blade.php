<div class="tab-pane fade active in" id="global_setting">
    ...
    <div class="col-md-10">
            ...
            <div class="form-group">
                ...
                <div class="col-md-9">
                    <input type="text" name="title[{{$code}}]" class="form-control" data-name="title.{{$code}}"
                           value="{{optional($course->translate($code))->title}}">
        ...
        @foreach (config('setting.locales') as $code)
            <div class="form-group">
                <label class="col-md-2">
                    {{__('dashboard.courses.create.description')}} - {{ $code }}
                </label>
                <div class="col-md-9">
                    <textarea name="description[{{$code}}]" rows="8" cols="80"
                              class="form-control {{check_dir($code)}}Editor"
                              data-name="description.{{$code}}">{!!optional($course->translate($code))->description!!}</textarea>
                    <div class="help-block"></div>
                </div>
            </div>
        @endforeach
        ...
        @foreach (config('setting.locales') as $code)
            <div class="form-group">
                ...
                <div class="col-md-9">
                    <textarea name="top_description[{{$code}}]" rows="8" cols="80"
                              class="form-control {{check_dir($code)}}Editor"
                              data-name="top_description.{{$code}}">{!!optional($course->translate($code))->top_description!!}</textarea>
                    <div class="help-block"></div>
                </div>
            </div>
        @endforeach
        ...
        <div class="form-group">
            ...
            <div class="col-md-9">
                    ...
                    <div class="video-card">
                        <div class="widget-thumb-wrap">
                            <div class="widget-thumb-body">
                                <a class="btn btn-primary" target="_blank" href="{{$course->demo_video ? url(route('leasons.show-video',$course->demo_video)) : ''}}">
                ...
                <input type="hidden" class="form-control" name="demo_video" id="demo_video" value="{{$course->demo_video}}">
                <div class="help-block"></div>
            </div>
        </div>
        <div class="form-group">
            ...
            <div class="col-md-9">
                <input type="checkbox" class="make-switch" id="test" data-size="small"
                       name="status" {{($course->status == 1) ? ' checked="" ' : ''}}>
                <div class="help-block"></div>
            </div>
        </div>
        <div class="form-group">
            ...
            <div class="col-md-9">
                ...
                <span id="holder" style="margin-top:15px;max-height:100px;">
                    <img src="{{url($course->image)}}" style="height: 15rem;">
                </span>
                <span id="image"></span>
            </div>
        </div>
        ...
        <div id="jstree">
            @include('dashboard.courses.tree.edit')
        </div>
    </div>
</div>
