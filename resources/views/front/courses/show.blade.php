...
@section('content')
            ...
            <div class="row">
                                    ...
                                    <div id="collapse{{$leason->id}}" class="lecture-list collapse show">
                                        <ul>
                                            @foreach ($leason->videos as $video)
                                                ...
                                                @else
                                                    <li class="lecture has-preview" data-toggle="modal"
                                                        data-target="#paidModal{{$video->id}}">
                                                        @endif
                ...
                <div class="col-lg-4">
                    <div class="course-sidebar">
                        ...
                        <form class="" action="{{url(route('front.cart.add'))}}" method="get">
                                ...
                                <div class="course-sidebar-text-box">
                                    @foreach ($course->packages as  $package)
                                        <div class="form-group check-box">
                                            <input type="radio" name="package" required value="{{$package->id}}"
                                            {{ $loop->first?'checked':'' }}

                                            >
                                            <label> {{ $package->title }} - {{ $package->price}} KWD </label>
                                        </div>
                                    @endforeach
                                    <div class="buy-btns">
                                        <button class="btn btn-add-cart"
                                                type="submit">{{__('front.courses.add_to_cart')}}</button>
                                    </div>
                                </div>
                        ...
                        @else
                            <div class="course-sidebar-text-box">
                                <div class="alert alert-primary">{{__('front.courses.free_course')}}</div>
                            </div>
                        @endif
    ...
    @foreach ($course->leasons as $leasonEmbed)
        @foreach ($leasonEmbed->videos as $videoEmbed)
            @if (!$videoEmbed->is_free)
                <div class="modal fade" id="paidModal{{$videoEmbed->id}}" tabindex="-1" role="dialog"
                     aria-labelledby="paidModalLabel" aria-hidden="true">
                    <div class="modal-dialog" role="document">
                        <div class="modal-content">
                            <div class="modal-body">
                                من فضلك اشترك لكي تتمكن من مشاهده جميع الدروس
                            </div>
    ...
    <script>
        ...
        function publishModel(video_id) {
            ...
            if (last_video_request !== video_id) {
                ...
                $.ajax({
                    type: "get",
                    url: '{{url('courses/free-lesson')}}/' + video_id,
